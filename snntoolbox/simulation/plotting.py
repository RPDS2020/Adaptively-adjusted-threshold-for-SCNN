...
def plot_convergence(cnn_acc,top1err_t,duration,dt,path=None):
    plt.figure()
    #    plt.title('Error vs simulation time')
    time = np.arange(0, duration, dt)
    Ann_acc = np.full(len(time),cnn_acc)
    plt.plot(time, top1err_t, 'b',label='SNN accuracy' )#
    # plt.plot(time, Ann_acc, 'r', label='ANN accuracy')
    plt.legend()
    plt.ylim(0, 100)
    plt.ylabel('Accuracy(%)')
    plt.xlabel('Simulation time(ms) '.format(dt))
    if path is not None:
        filename = 'Accuracy_vs_time'
        plt.savefig(os.path.join(path, filename), bbox_inches='tight')
    else:
        plt.show()
    plt.close()
