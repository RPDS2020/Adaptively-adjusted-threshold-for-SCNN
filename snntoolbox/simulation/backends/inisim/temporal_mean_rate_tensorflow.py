...
class SpikeLayer(Layer):
    ...
    def linear_activation(self, mem):
        """Linear activation."""
        return k.cast(k.greater_equal(mem, self.dyn_v_thresh), k.floatx()) * \
            self.v_thresh
       # return k.cast(k.greater_equal(mem, self.dyn_v_thresh), k.floatx()) * self.dyn_v_thresh
    ...
    def quantized_activation(self, mem, m, f):
        ...
        return k.cast(k.greater_equal(mem, self.v_thresh), k.floatx()) * \
            self.v_thresh
    def A(self,dyn_v_thresh_):
       # self.add_update([(self.dyn_v_thresh, dyn_v_thresh_)])
        #tf.compat.v1.assign(self.dyn_v_thresh,dyn_v_thresh_) 
        k.update(self.dyn_v_thresh,dyn_v_thresh_)
        return 1
    def B(self):
        return 2
    def get_new_mem(self):
        ...
        max_value = k.max(masked_impulse)
    #    self.dyn_v_thresh  = self.v_thresh    
        dyn_v_thr_ = tf.cond(k.greater_equal(max_value , self.dyn_v_thresh),
                              lambda: max_value,
                              lambda: self.dyn_v_thresh)
                                  
        dyn_v_thresh_ = tf.cond(k.less_equal(dyn_v_thr_   , self.v_thresh ),
                              lambda: dyn_v_thr_  ,
                              lambda: self.v_thresh )  
        #tf.compat.v1.assign(self.dyn_v_thresh,dyn_v_thresh) 
        ab_thr = tf.cond(k.greater(dyn_v_thresh_ , self.dyn_v_thresh),
                              lambda: self.A(dyn_v_thresh_) ,
                              lambda: self.B())        
    ...
    def reset_spikevars(self, sample_idx):
        ...
        if do_reset:
           ...
           # self.add_update([(self.dyn_v_thresh, self._v_thresh *0.5)])
            k.set_value(self.dyn_v_thresh,self._v_thresh *0.7)
        if self.tau_refrac > 0:
    ...
    def init_neurons(self, input_shape):
        ...
        self.dyn_v_thresh = k.variable(self._v_thresh *0.7 , name='dyn_v_thresh')
