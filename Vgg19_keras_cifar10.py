...
# Keras Version: 2.3.1
# dataset: cifar10
# Result: test accuracy about 93.40 ~ 94.50%
# ========================================================== #
import os
import sys
import datetime
...
def save_file(name):
    desk=os.path.abspath(os.path.join(os.path.dirname(os.path.realpath(
    __file__)), '.'))
    fullpath=desk+name+'.txt'
    file=open(fullpath,'w')
filename='ANN_Cifar10'
save_file(filename)
output=sys.stdout
outputfile=open(os.path.abspath(os.path.join(os.path.dirname(os.path.realpath(
    __file__)), '.'))+filename+'.txt','w')
...
dropout      = 0.5
weight_decay = 0.0001
log_filepath = os.path.join('vgg19_retrain_logs—1')
...
checkpoint = ModelCheckpoint('best_model_improved-1.h5',  # model filename
